...
当前 `quant_factor_lab` 已提供 evidence preview 入口：研究员可以按 `algorithm_id + gate_id + evidence_type + evidence_source + summary` 提交证据，服务会校验算法和 gate 是否存在，并返回标准 `AlgorithmReviewGateEvidenceRecord`。101 真实因子流转 smoke 已能把 `quant_factor_validation` 生成的 validation artifact 自动映射为 `technical.momentum / validation_evidence` gate evidence preview。该接口当前不持久化、不修改 gate 状态，只用于固定提交格式和后续持久化接口的协议。
