...
| 能力 | 第一版处理方式 | 外部库状态 |
| ---- | ---- | ---- |
| 股票截面验证 | 先使用自研 `quant_factor_validation` | Alphalens payload runner 边界已落地；Qlib 作为后续 benchmark adapter |
...
当前代码已经完成上述第一阶段协议的基础落地，并在 `quant_factor_validation` 中输出 `internal` 引擎的 `FactorScoreCard`、`FactorEvaluationResult` 和 `FactorComparisonReport`。外部库已落地标准摘要 adapter 入口，并提供 Alphalens payload runner 边界；Qlib / vectorbt 等具体 runner 尚未作为运行依赖接入。
...
后续补强点是：为 Alphalens 补第三方库执行层，为 Qlib / vectorbt 分别补具体 runner，把这些库的原始输出整理成 `ExternalFactorValidationSummary`，再通过现有 adapter 进入 `FactorEvaluationResult` 和 `FactorComparisonReport`。

本阶段建议接入方式：

| 方向 | 推荐库 | 使用方式 |
| ---- | ---- | ---- |
| 股票截面验证 | Alphalens、Qlib | Alphalens 已有 payload runner 边界；Qlib 后续作为 benchmark adapter |
