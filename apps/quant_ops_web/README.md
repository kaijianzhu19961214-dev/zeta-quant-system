...
当前外部引擎对比区读取 BFF 提供的 MVP 预览结果，用于验证 UI -> BFF -> `quant_factor_validation` 链路；同时会展示 `artifact_reference`，用于定位后续应读取的标准 `comparison_report.json`。当前尚未读取对象内容，后续接入真实研究任务后，应通过只读 object-store adapter 读取标准 artifact 或研究员提交的已审核 payload。
