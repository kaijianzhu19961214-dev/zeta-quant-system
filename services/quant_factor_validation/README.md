...
评分卡当前在线 API 仍使用 `internal` 规则评分引擎，输出透明 `score_components`、`final_score` 和 `review_decision`。服务层已提供 `ExternalFactorValidationSummary -> FactorEvaluationResult` 的标准 adapter，并已落地 `AlphalensMetricSummary -> ExternalFactorValidationSummary` normalizer；Alphalens、Qlib、vectorbt、OpenSourceAP/CrossSection 和 commodity-curve-factors 当前不作为运行依赖。
...
- Alphalens 输出当前只接入统计摘要 normalizer；完整 runner 必须继续隔离在 adapter 层，不能把 notebook / 第三方对象直接写进核心协议。
