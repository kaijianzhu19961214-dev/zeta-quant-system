    ...
    async def get_ingestion_ledger_preview(self, *, limit: int = 100) -> MarketDataIngestionLedgerPreview:
        try:
            async with httpx.AsyncClient(
                ...
                response = await client.get(
                    f"{self.base_url}/api/v1/ingestion/ledger/preview",
                    params={"limit": limit},
                )
        except httpx.TimeoutException as error:
            raise QuantDataHubClientError(
                status_code=504,
                message="quant data hub ingestion ledger preview request timed out",
            ) from error
        except httpx.HTTPError as error:
            raise QuantDataHubClientError(
                status_code=502,
                message=f"quant data hub ingestion ledger preview request failed: {error}",
            ) from error

        if response.status_code < 200 or response.status_code >= 300:
            raise QuantDataHubClientError(
                status_code=502,
                message=f"quant data hub ingestion ledger preview request returned status {response.status_code}",
            )

        return _parse_ingestion_ledger_preview_response(response=response)
...
def _parse_ingestion_ledger_preview_response(*, response: httpx.Response) -> MarketDataIngestionLedgerPreview:
    payload = _safe_json(response=response)
    if not isinstance(payload, dict):
        raise QuantDataHubClientError(
            status_code=502,
            message="quant data hub ingestion ledger preview response is not a JSON object",
        )

    try:
        return MarketDataIngestionLedgerPreview.model_validate(payload)
    except ValueError as error:
        raise QuantDataHubClientError(
            status_code=502,
            message="quant data hub ingestion ledger preview response does not match the contract",
        ) from error
