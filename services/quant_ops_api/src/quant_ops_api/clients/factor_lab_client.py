        ...
        *,
        ...
        try:
            async with httpx.AsyncClient(
                ...
                response = await client.get(
                    f"{self.base_url}/api/v1/algorithms/{algorithm_id}/promotion/readiness",
                    params={"limit": limit},
                )
        except httpx.TimeoutException as error:
            raise FactorLabClientError(
                status_code=504,
                message="factor lab promotion readiness request timed out",
            ) from error
        except httpx.HTTPError as error:
            raise FactorLabClientError(
                status_code=502,
                message=f"factor lab promotion readiness request failed: {error}",
            ) from error

        if response.status_code < 200 or response.status_code >= 300:
            raise FactorLabClientError(
                status_code=502,
                message=f"factor lab promotion readiness returned status {response.status_code}",
            )
...
def _parse_algorithm_promotion_readiness(
    *,
    response: httpx.Response,
) -> AlgorithmPromotionReadinessResponse:
    payload = _safe_dict_json(response=response)
    if payload is None:
        raise FactorLabClientError(
            status_code=502,
            message="factor lab promotion readiness response is not valid JSON",
        )

    try:
        return AlgorithmPromotionReadinessResponse.model_validate(payload)
    except ValueError as error:
        raise FactorLabClientError(
            status_code=502,
