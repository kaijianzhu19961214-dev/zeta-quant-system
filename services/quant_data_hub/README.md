...
Tushare 全市场日线导入 / Full-market daily import：

```bash
export TUSHARE_TOKEN="<your_local_token>"
export TUSHARE_PROXY_BASE_URL="https://tt.xiaodefa.cn"

python3 scripts/import_tushare_a_share_daily.py \
  --start-date 20240101 \
  --end-date 20260630 \
...
- 按交易日读取 Tushare `trade_cal`、`daily` 和 `adj_factor`。
- 将 A 股日线 raw 行情写入 101 ClickHouse `quant_market.market_data_1d_raw`。
- 使用 `source_name=tushare_proxy` 和 `dataset_code=a_share_1d` 标识数据来源。
- 写入前按 `dataset_code + source_name + date` 检查已有数据，默认跳过已存在交易日，支持中断后重跑。
- 本机只保存 `.gitignore` 排除的进度日志，不保存全量行情明细。

当前存储边界：

```text
ClickHouse:
  股票行情明细主存储，可通过 quant_data_hub API 查询。

PostgreSQL:
  控制面存储，后续保存导入批次、任务、血缘和质量检查结果。
  当前脚本不把全量行情明细复制到 PostgreSQL。

MinIO:
  对象归档存储，后续保存原始响应、Parquet 快照和因子产物。
  当前脚本不写 MinIO。
```

2026-07-02 已完成一次真实导入：

```text
range: 2024-01-02 ~ 2026-06-30
trading_days: 601
rows: 3,244,082
symbols: 5,620
duplicates(code, date): 0
storage: 101 ClickHouse / quant_market.market_data_1d_raw
```

覆盖率查询 / Source coverage：
...
- 按 `timeframe / dataset_code / source_name` 聚合 ClickHouse raw 行情表。
- 返回行数、标的数、交易日数、日期范围和 `code + date` 或 `code + trade_time` 重复键行数。
- 给 `quant_ops_api` 和 `quant_ops_web` 做只读监控，不读取大规模明细到浏览器。

导入账本预览 / Ingestion ledger preview：
...
- 从 ClickHouse source coverage 派生导入批次和质量检查记录。
- 返回 `ingestion_runs`、`ingestion_quality_checks` 的预览结构，当前 `persistence_status=not_persisted`。
- 用于先验证控制面协议，不在本轮直接写入 101 PostgreSQL。
- PostgreSQL 账本表结构草案位于 `infra/postgres/init/03_create_data_hub_ingestion_ledger.sql`，包含 `ingestion_runs`、`ingestion_quality_checks`、`ingestion_artifacts`。
